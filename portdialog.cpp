...
PortDialog::PortDialog(QVariantMap param,QWidget *parent): QDialog(parent), paramMap(param) ,ui(new Ui::PortDialog), m_autoDetect(new AutoDetectManager(this))
...
void PortDialog::setupConnections()
{
    ...
    connect(ui->btnClearLog, &QPushButton::clicked,
            ui->textLog, &QPlainTextEdit::clear);
}

void PortDialog::onDebuggerDetected(const QString &portName, const QString &description)
{
...
void PortDialog::onDebuggerRemoved(const QString &portName)
{
...
void PortDialog::onSerialConnected(const QString &portName)
{
    m_isConnected = true;
...
void PortDialog::onSerialDisconnected()
{
    m_isConnected = false;
    updateConnectionStatus(false);
...
void PortDialog::on_btnManualRefresh_clicked()
{
    refreshPortList();
    logMessage("手动刷新设备列表");
}

void PortDialog::on_btnConnect_clicked()
{
...
void PortDialog::on_btnDisconnect_clicked()
{
    m_serialManager->disconnectPort();
}


void PortDialog::refreshPortList()
{
...
void PortDialog::logMessage(const QString &message, bool isError)
{
...
void PortDialog::updateConnectionStatus(bool connected)
{
